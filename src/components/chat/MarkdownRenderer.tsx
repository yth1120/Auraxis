    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="font-body text-lg leading-[1.75] text-text-primary break-words [&_p]:mb-2.5 [&_p:last-child]:mb-0 [&_p]:text-pretty [&_:is(h1,h2,h3,h4,h5,h6)]:font-heading [&_:is(h1,h2,h3,h4,h5,h6)]:font-semibold [&_:is(h1,h2,h3,h4,h5,h6)]:text-text-primary [&_:is(h1,h2,h3,h4,h5,h6)]:my-4 [&_:is(h1,h2,h3,h4,h5,h6)]:mb-2 [&_:is(h1,h2,h3,h4,h5,h6)]:tracking-tight [&_h1]:text-[24px] [&_h1]:leading-[34px] [&_h2]:text-[22px] [&_h2]:leading-[32px] [&_h3]:text-[20px] [&_h3]:leading-[30px] [&_h4]:text-[16px] [&_h4]:leading-[28px] [&_ul]:my-2 [&_ol]:my-2 [&_ul]:pl-5 [&_ol]:pl-5 [&_li]:my-1 [&_li::marker]:text-text-secondary [&_img]:max-w-full [&_img]:rounded-md [&_strong]:font-semibold">
          <pre className="whitespace-pre-wrap break-words font-mono text-sm text-text-secondary bg-bg-tertiary rounded-md p-3 max-h-100 overflow-y-auto">{this.props.content}</pre>
        </div>
      );
    }
  ...
  return (
    <KatexErrorBoundary content={content}>
      <ReactMarkdown
        ...
        components={{
          h1({ children }) {
            return <h1 className="mt-8 mb-4 font-heading text-[24px] leading-[34px] font-bold tracking-tight">{children}</h1>;
          },
          h2({ children }) {
            return <h2 className="mt-8 mb-4 font-heading text-[22px] leading-[32px] font-bold tracking-tight">{children}</h2>;
          },
          h3({ children }) {
            return <h3 className="mt-8 mb-4 font-heading text-[20px] leading-[30px] font-bold tracking-tight">{children}</h3>;
          },
          h4({ children }) {
          ...
          code({ node, className, children, ...props }) {
            ...
            return (
              <code className="bg-accent-soft border border-accent-border rounded-md px-1.5 py-px text-[0.88em] text-accent font-mono" {...props}>
                {children}
              </code>
            );
          },
          ...
          blockquote({ children }) {
            return <blockquote className="border-l-2 border-l-[var(--color-text-faint)] my-4 pl-3.5 text-text-secondary">{children}</blockquote>;
          },
          a({ href, children }) {
            return (
              <a href={href} target="_blank" rel="noopener noreferrer" className="text-accent no-underline border-b border-accent-border transition-colors duration-150 ease-out hover:text-accent-hover hover:border-b-accent">
                {children}
              </a>
            );
          },
