    ...
    <div className="flex-1 flex flex-col overflow-hidden relative">
      {searchOpen && (
        <div className="ax-search-bar shrink-0" style={{ paddingTop: headerInset }}>
          ...
          />
          {totalMatches > 0 && (
            <span className="font-mono text-xs text-[var(--color-text-secondary)] whitespace-nowrap min-w-[60px] text-center">{matchIndex + 1}/{totalMatches}</span>
          )}
          <span className="flex gap-1">
          <span className="p-1 rounded-[5px] cursor-pointer text-[var(--color-text-muted)] transition-colors duration-150 ease-out hover:bg-[var(--color-accent-soft)] hover:text-[var(--color-text-primary)]" onClick={() => navigateMatch(-1)} aria-label={t('msglist.prevMatch')} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigateMatch(-1)}>
              <UpOutlined />
            </span>
          <span className="p-1 rounded-[5px] cursor-pointer text-[var(--color-text-muted)] transition-colors duration-150 ease-out hover:bg-[var(--color-accent-soft)] hover:text-[var(--color-text-primary)]" onClick={() => navigateMatch(1)} aria-label={t('msglist.nextMatch')} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigateMatch(1)}>
              <DownOutlined />
            </span>
          </span>
          <span className="text-[var(--color-text-muted)] cursor-pointer text-sm p-1 rounded-[5px] transition-colors duration-150 ease-out hover:text-[var(--color-text-primary)]" onClick={() => setSearchOpen(false)} aria-label={t('msglist.closeSearch')} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && setSearchOpen(false)}>
            <CloseOutlined />
          </span>
        </div>
      )}
      ...
      {!isAtBottom && messages.length > 0 && (
        <button
          ...
          style={{ left: 'calc(50% + var(--content-max-width, 880px) / 2 - 56px)', bottom: `${Math.max(0, bottomInset) + 20}px` }}
