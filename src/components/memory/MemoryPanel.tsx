...
export default function MemoryPanel() {
  ...
  const rejections = useMemoryStore((s) => s.rejections);
    ...
    return (
      <div className="mt-2 pt-2 border-t border-[var(--color-border-dim)]">
        <div className="flex items-center gap-2 mb-2">
          ...
          {audit.belief.legacy === 1 && (
            <Tag style={{ fontSize: 10 }}>{t('mem.audit.legacy')}</Tag>
          )}
        ...
        {audit.evidence.length === 0 ? (
          ...
          <div className="space-y-2">
            {audit.evidence.slice(0, 5).map((item) => (
              <div key={item.evidence.id} className="px-2 py-1.5 rounded-md bg-[var(--color-bg-secondary)] border border-[var(--color-border-dim)]">
                <div className="text-2xs text-muted mb-1">
                  [{item.evidence.role}] {new Date(item.evidence.ts).toLocaleString()} · {t('mem.audit.support')} {item.support_strength.toFixed(2)}
                </div>
                <p className="m-0 text-xs leading-[1.6] text-secondary break-words">{item.evidence.content.slice(0, 300)}</p>
                {item.signals.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {item.signals.slice(0, 6).map((s) => (
                      <Tag key={s.id} style={{ fontSize: 9 }}>{s.signal_type}: {s.value.slice(0, 24)}</Tag>
                    ))}
                  </div>
                )}
              </div>
        ...
        evidenceItems.map((e) => {
          ...
          return (
            ...
            >
              ...
              {isExpanded && (
                <div className="mt-2 pt-2 border-t border-[var(--color-border-dim)]">
                  <p className="m-0 mb-2 text-xs leading-[1.6] text-secondary whitespace-pre-wrap break-words">{e.content}</p>
                  <div className="text-2xs text-muted">hash: {e.content_hash.slice(0, 16)}… · {e.session_id || '-'}</div>
                </div>
              )}
            </div>
          );
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3">
        <div className="flex gap-2 mb-3">
          <Input
            ...
            size="small"
          />
          <Button size="small" type="primary" loading={diagRunning} onClick={handleRunDiag} icon={<GaugeOutlined size={14} />}>
            {t('mem.diag.run')}
          </Button>
        </div>
        {!lastReadResult ? (
            ...
            <div className="px-3 py-2 rounded-md bg-secondary border border-dim">
              ...
              {diag!.routes.map((r) => (
                <div key={r.route} className="flex items-center justify-between text-xs py-0.5">
                  <span className="text-secondary">{r.route}{r.skipped ? ' (skipped)' : ''}</span>
                  <span className="text-muted">{r.hits} hits · {r.latencyMs}ms</span>
                </div>
              ))}
              <div className="flex items-center justify-between text-xs py-0.5 border-t border-[var(--color-border-dim)] mt-1 pt-1">
                ...
                <span className="text-muted">{diag!.budget.used}/{diag!.budget.allocated} tokens{diag!.budget.truncated ? ' · truncated' : ''}</span>
              </div>
            </div>
            <div className="px-3 py-2 rounded-md bg-secondary border border-dim">
              ...
              {[
                ...
                <div key={String(flagKey)} className="flex items-center justify-between text-xs py-0.5">
                  ...
                  <span className={flagValue ? 'text-[var(--color-danger)]' : 'text-[var(--color-success)]'}>{flagValue ? 'true' : 'false'}</span>
                </div>
              ))}
            </div>
            {lastReadResult.context.length > 0 && (
              <div className="px-3 py-2 rounded-md bg-secondary border border-dim">
                ...
                {lastReadResult.facts.slice(0, 10).map((f, i) => (
                  <p key={i} className="m-0 text-xs text-secondary leading-[1.6] break-words">{f}</p>
                ))}
              </div>
            )}
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className={clsx('inline-flex items-center gap-1 rounded-full h-5 px-1.5 text-2xs font-medium border', cfg.chip)}>
