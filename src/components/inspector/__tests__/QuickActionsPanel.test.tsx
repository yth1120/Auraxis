...
describe('QuickActionsPanel', () => {
  ...
  it('renders one launch tile per built-in skill without a section header', () => {
    ...
    expect(buttons).toHaveLength(AGENT_SKILLS.length);
    expect(buttons[0].textContent).toContain(AGENT_SKILLS[0].name);
  });
  ...
  it('fills the composer with the skill prompt for manual review', () => {
    ...
    expect(useChatStore.getState().inputValue).toBe(AGENT_SKILLS[0].instruction);
