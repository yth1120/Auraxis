...
function ToolDetail({ entry }: { entry: AgentLogEntry }) {
  ...
  if (isBash) {
    ...
    const content = entry.streamOutput
      ? entry.streamOutput
  ...
  if (entry.toolName === 'Grep' || entry.toolName === 'Glob') {
    ...
    return <AgentSearchCard kind="matches" files={[...byFile.values()]} total={results.length} truncated={o.truncated === true} />;
  }
  if (entry.toolName === 'WebFetch' || entry.toolName === 'WebSearch') {
    ...
    if (entry.toolName === 'WebSearch') {
      ...
      return <AgentWebCard kind="search" sources={results.map((r) => ({ url: r.url ?? '', title: r.title, snippet: r.snippet }))} />;
    }
    return (
      <AgentWebCard
        kind="fetch"
...
function TrajectoryToolRow({
  ...
  const isFile = entry.toolName === 'Read' || entry.toolName === 'Write' || entry.toolName === 'Edit' || entry.toolName === 'NotebookEdit';
  ...
  return (
    <>
      <tr
        ...
        onClick={onSelect}
        onDoubleClick={(e) => { e.stopPropagation(); onJump(); }}
      >
        ...
        <td className="w-20 px-1">
          <span className={clsx(
            ...
            running ? 'text-primary bg-primary-soft' : failed ? 'text-danger bg-danger-soft' : 'text-success bg-success-soft',
          )}>
        ...
        <td className="px-2 min-w-0">
          {filePath ? (
            <button
              ...
              onClick={(e) => { e.stopPropagation(); useAppStore.getState().requestOpenFile(filePath); }}
              title={filePath}
            >
              {summary || basename(filePath)}
            </button>
          ) : (
            <span className={clsx('block min-w-0 overflow-hidden text-ellipsis whitespace-nowrap font-mono', failed ? 'text-danger' : 'text-text-muted')}>
...
export default function TimelinePanel() {
  ...
  const turns = useMemo<Turn[]>(() => {
    ...
    for (const e of agent?.log ?? []) {
      ...
      if (e.type === 'tool_end' || e.type === 'tool_error') {
        const idx = cur.entries.findIndex(
          (x) => x.type === 'tool_start' && x.toolCallId === e.toolCallId,
        );
        if (idx >= 0) cur.entries[idx] = e;
        else cur.entries.push(e);
        continue;
      }
      if (e.type === 'text' || e.type === 'thinking' || e.type === 'warning' || e.type === 'error' || e.type === 'progress') {
        cur.entries.push(e);
      }
    }
    return list;
    ...
    const key = (() => {
      for (const turn of turns) {
        const found = turn.entries.find(
          (e) => (e.type === 'tool_start' || e.type === 'tool_end' || e.type === 'tool_error')
  ...
  const hasAnyError = agent?.log.some(
  ...
  const toolNames = useMemo(() => {
    ...
    for (const turn of turns) {
      for (const entry of turn.entries) {
        if ((entry.type === 'tool_start' || entry.type === 'tool_end' || entry.type === 'tool_error') && entry.toolName) {
              ...
              <div className="flex items-center rounded-md border border-border-default overflow-hidden">
                <button
                  ...
                  className={clsx(
                    ...
                    viewMode === 'table' ? 'bg-primary-soft text-primary' : 'bg-transparent text-text-muted hover:bg-[var(--color-hover)] hover:text-text-secondary',
                  )}
                ...
                <button
                  ...
                  className={clsx(
                    'h-6 px-2 text-xs font-medium border-none cursor-pointer transition-colors duration-150',
                    viewMode === 'timeline' ? 'bg-primary-soft text-primary' : 'bg-transparent text-text-muted hover:bg-[var(--color-hover)] hover:text-text-secondary',
                  )}
            ...
            {([['all', tPanel('tl.filterAll')], ['running', tPanel('tl.filterRunning')], ['failed', tPanel('tl.filterFailed')], ['done', tPanel('tl.filterDone')]] as const).map(([key, label]) => (
              <button
                ...
                className={clsx(
                  ...
                  filter === key ? 'bg-primary-soft text-primary' : 'text-text-muted hover:bg-[var(--color-hover)] hover:text-text-secondary',
                )}
          ...
          <tbody>
            ...
            {flatRows.rows.slice(range.start, range.end).map((row) => {
                ...
                return (
                  ...
                  >
                    <td colSpan={4} className="px-3">
                      <span className="text-xs font-semibold text-text-secondary">{tPanel('timeline.round', { n: row.turn.iteration })}</span>
              ...
              if (row.kind === 'plain' && row.entry) {
                ...
                const text = entry.type === 'text' || entry.type === 'thinking'
                  ? (entry.text ?? '')
                  : entry.type === 'warning' || entry.type === 'error'
                    ? (entry.text ?? entry.error ?? '')
                    : (entry.text ?? '');
                return (
                  <tr key={row.key} data-row-key={row.key} style={{ height: ROW_H }} className="transition-colors duration-100">
                    ...
                    <td className="w-20 px-1">
                      <span className={clsx(
                        ...
                        entry.type === 'error' ? 'text-danger bg-danger-soft' : entry.type === 'warning' ? 'text-warning bg-warning-soft' : entry.type === 'thinking' ? 'text-primary bg-primary-soft' : 'text-text-muted bg-[var(--color-bg-inset)]',
                      )}>
                        {entry.type === 'thinking' ? 'Think' : entry.type === 'warning' ? 'Warn' : entry.type === 'error' ? 'Error' : entry.type === 'progress' ? 'Progress' : 'Message'}
                      </span>
                    </td>
              ...
              return (
                <tr key={row.key} style={{ height: ROW_H }}>
                  <td colSpan={4} className="px-2 py-1.5 text-xs text-text-faint">{tPanel('timeline.noTools')}</td>
                </tr>
              );
            })}
              ...
              <>
                ...
                {timelineRows.map(({ turn, items, total }) => (
                  <div key={turn.iteration} className="flex items-center gap-2 py-[3px]">
                      ...
                      {items.map(({ entry, dur }, idx) => (
                        <button
                          ...
                          className={clsx(
                            ...
                            entry.type === 'tool_error' ? 'bg-danger' : entry.type === 'tool_start' ? 'bg-accent' : 'bg-success',
                          )}
                    ...
                    <span className="w-16 shrink-0 text-right text-2xs text-text-faint tabular-nums">{fmtDuration(total)}</span>
                  </div>
                ))}
              </>
            )}
      ...
      {selectedEntry && (
        <aside className="w-[340px] shrink-0 border-l border-border-dim bg-[var(--color-bg-elevated)] flex flex-col min-h-0">
          <header className="flex items-center gap-2 h-10 px-3 border-b border-border-dim shrink-0">
            <StateDot
              state={selectedEntry.type === 'tool_error' ? 'error' : selectedEntry.type === 'tool_start' ? 'ongoing' : 'done'}
              className="shrink-0"
            />
            <span className="font-medium text-xs text-text-primary">{selectedEntry.toolName}</span>
            <span className="ml-auto text-xs text-text-muted tabular-nums">{fmtDuration(selectedEntry.durationMs)}</span>
            <button
              type="button"
            ...
            {(['overview', 'input', 'output', 'timing'] as const).map((tab) => (
              <button
                ...
                className={clsx(
                  ...
                  detailTab === tab ? 'bg-primary-soft text-primary' : 'text-text-muted hover:bg-[var(--color-hover)] hover:text-text-secondary',
                )}
