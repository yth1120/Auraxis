...
export default function TerminalBlock({
  ...
  return (
    <div
      className={clsx(
        'terminal-block font-mono text-xs leading-[1.55] text-text-primary',
        className,
      )}
    ...
    >
      <div className="terminal-block-header">
        <div className="terminal-block-prompt">
          {commandLines.map((line, i) => (
            <div key={i} className="terminal-block-prompt-line">
              ...
              <span className="terminal-block-cwd shrink-0">
                {i === 0 && cwd ? promptLabel(cwd, home) : '$'}
              </span>
        ...
        {statusText !== '' && (
          <span className="terminal-block-pill shrink-0">{statusText}</span>
        )}
        {!running && visible && (
          <button
            type="button"
