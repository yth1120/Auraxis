...
const CODE_EXTS = new Set(['.ts', '.tsx', '.js', '.jsx', '.py', '.rs', '.go', '.java', '.c', '.cpp', '.h', '.vue', '.svelte']);
...
export default function FileTree({ onFileSelect }: FileTreeProps) {
  ...
  const handleDelete = useCallback(async (targetPath: string, isDir: boolean) => {
  ...
  const handleStartCreate = useCallback((parentPath: string, type: 'createFile' | 'createFolder') => {
  ...
  }, [activeOp, inputValue, projectRoot, refresh]);

  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Enter') handleFinishOp();
    if (e.key === 'Escape') { setActiveOp(null); setInputValue(''); }
  }, [handleFinishOp]);
  ...
  const renderActions = (entryPath: string, entryName: string, isDir: boolean) => {
    ...
    <Tooltip title={t('ft.rename')} placement="top">
      ...
      title={isDir ? t('ft.deleteDirTitle') : t('ft.deleteFileTitle')}
          onConfirm={(e) => { e?.stopPropagation(); handleDelete(entryPath, isDir); }}
  ...
  const renderNode = useCallback((entry: DirectoryEntry, depth: number) => {
    ...
    const nodeBaseClasses = 'flex items-center gap-0.5 w-full py-[3px] px-2 border-none bg-transparent text-secondary text-xs cursor-pointer text-left transition-colors duration-150 ease-out leading-[1.6] min-h-6 overflow-hidden relative hover:bg-[var(--color-hover)]';
    ...
    if (entry.isDirectory) {
      return (
        <div key={entry.path}>
          ...
          >
            <span className={`inline-flex shrink-0 items-center justify-center w-3 h-3 text-2xs text-faint transition-transform duration-normal ease-out ${isExpanded ? 'rotate-90' : ''}`}>
              <RightOutlined />
            </span>
          ...
          {isExpanded && entry.children && (
            <div>
              {activeOp && activeOp.parentPath === entry.path && (activeOp.type === 'createFile' || activeOp.type === 'createFolder') && (
                <div
                  className={nodeBaseClasses}
                  style={{ paddingLeft: 8 + indent + 12 }}
                >
                  <span className="text-xs w-4 shrink-0 inline-flex items-center justify-center text-faint">
                    {activeOp.type === 'createFolder' ? <FolderOutlined /> : fileIcon(inputValue || 'new.ts', false)}
                  </span>
                  <Input
                    size="small"
                    value={inputValue}
    ...
    return (
      <div
        ...
        onMouseEnter={() => setHoveredPath(entry.path)}
        onMouseLeave={() => setHoveredPath(null)}
        onClick={() => { if (!isRenaming) onFileSelect?.(entry.path); }}
        title={entry.path}
      >
        <span className="text-xs w-4 shrink-0 inline-flex items-center justify-center text-faint">
  ...
  }, [expandedPaths, toggleExpand, activeOp, inputValue, hoveredPath, fileStatus, handleFinishOp, handleKeyDown, handleDelete, handleStartRename, handleStartCreate]);
  ...
  return (
    <div className="file-tree flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between px-3 py-1.5 shrink-0">
        <span className="text-2xs font-semibold text-muted uppercase tracking-[0.05em] overflow-hidden text-ellipsis whitespace-nowrap flex-1">{tree.name}</span>
