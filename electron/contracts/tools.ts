...
export type BuiltInToolName =
  ...
  | 'TodoWrite' | 'Agent' | 'Replan'
  | 'CronCreate' | 'CronDelete' | 'CronList'
  | 'TaskOutput' | 'TaskStop'
  | 'EnterPlanMode' | 'ExitPlanMode'
  | 'NotebookEdit'
  | 'EnterWorktree' | 'LSP' | 'ReviewArtifact'
  | 'ListSkills' | 'ReadSkill' | 'SessionQuery'
  | 'ReadSpill'
  | 'RunWorkflow' | 'RunCode'
  | 'AskUser' | 'Pty'
  | 'InspectRuntime' | 'WriteSkill'
  | 'ListAgents' | 'SendMessage' | 'InterruptAgent' | 'Report'
  | 'GetGoal' | 'CreateGoal' | 'UpdateGoal'
  | 'MountPlugin' | 'UnmountPlugin'
  | 'Ralph' | 'Pwsh'
  | 'SessionEventSearch' | 'SessionEventRead' | 'SessionTrace'
  | 'TaskList';
